...
TEST_CASE_METHOD(LispTest, "AndSyntax") {
    ...
    // that evaluates to a false value is returned. ValueType remaining expressions are not evaluated.
...
TEST_CASE_METHOD(LispTest, "OrSyntax") {
    // (or <test>)
    // The <test> expressions are evaluated from left to right, and the value of the first expression
    // that evaluates to a true value is returned. ValueType remaining expressions are
